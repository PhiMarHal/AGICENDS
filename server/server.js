...
const SIM = {
  ...
  // Polygon obstacle configs — match single-player exactly.
  PENTAGON: {
    startInterval: 5,
    spawnRate: [2, 4],
    size: 240,
    moveSpeed: 80,
    rotationRange: 60,    // degrees
    rotationSpeed: 0.001, // sin frequency
  },
  HEXAGON: {
    startInterval: 9,
    spawnRate: [2, 4],
...
function isSpaceClear(world, x, y, minDist) {
  ...
  // Larger clearance around polygon obstacles.
...
// Probability-based per-chunk spawning of polygon obstacles.
function maybeSpawnPolygonObstacle(world, intervalNumber, chunkTop, chunkBottom, minX, maxX, elapsedMs) {
  ...
  for (const { name, cfg } of types) {
    ...
    // Skip if too close to an interval barrier.
    const startY = SIM.CANVAS_HEIGHT - SIM.START_Y_OFFSET;
    let bad = false;
    for (const intervalAlt of world.intervalAltitudes) {
      const intervalY = startY - intervalAlt;
...
// Generic AABB-style bounce off rectangular blocks.
...
// Circle-vs-circle bounce (used for pentagons, heptagons).
...
function step(match, deltaSeconds) {
  ...
  // Pre-start: process flaps; nothing else moves.
  if (!match.hasStarted) {
    for (const p of match.players.values()) {
  ...
  // World scrolls: spikes rise, generation extends.
  ...
  // Update polygon obstacles (movement, state machines).

  // Pentagons: fall straight down, oscillate angle.
  ...
  // Hexagon pairs: state machine + beam collision.
  ...
  for (const pair of w.hexagonPairs.values()) {
    ...
    if (cycleTime < SIM.HEXAGON.rechargeDuration - SIM.HEXAGON.spinDuration) {
      ...
      pair.leftAngle = spinAngle;
      pair.rightAngle = -spinAngle;
      pair.beamActive = false;
    } else {
      pair.state = 'firing';
      ...
      // Pulse the beam height between 20 and 30, 4 cycles over fire duration.
      const pulseProgress = fireProgress * 4 * Math.PI * 2;
      pair.beamHeight = 25 + Math.sin(pulseProgress) * 5;
    }
  }

  // Heptagons: bounce between walls, fall.
  for (const h of w.heptagons.values()) {
    ...
    h.angle += 120 * deltaSeconds; // visual spin
  }

  // Octagons: shoot at nearest alive player below within range.
  for (const o of w.octagons.values()) {
    if (o.pulseTimer > 0) {
      o.pulseTimer -= deltaSeconds * 1000;
      // Sharp pulse twice over 400ms — simplified to scale tween.
    ...
    for (const pl of match.players.values()) {
      ...
      if (pl.y <= o.y) continue; // only fire at players below
  ...
  for (const p of match.players.values()) {
    ...
    // Pentagon collision (no damage to pentagon, just bounce).
    for (const pent of w.pentagons.values()) {
      resolveCircleBounce(p, pent.x, pent.y, SIM.PENTAGON.size / 2, match.eventsThisTick);
    }

    // Hexagon body collision (no damage).
    for (const pair of w.hexagonPairs.values()) {
    ...
    // Hexagon beam collision (stuns + boosts bounce).
    for (const pair of w.hexagonPairs.values()) {
  ...
  // Cleanup: drop entities that fell well below the spike line.
...
function buildSnapshot(match) {
  ...
  // Per-tick state for moving polygon obstacles. Sent every tick.
  ...
  for (const proj of match.world.projectiles.values()) {
    projectileStates[proj.id] = { x: proj.x, y: proj.y, scale: proj.scale };
  }
...
setInterval(() => {
  ...
  if (logTimer >= 1000) {
    ...
    if (match.players.size > 0) {
      const w = match.world;
      const lines = [
        `t=${(match.elapsedMs/1000).toFixed(1)}s spikeY=${match.spikeY.toFixed(0)} interval=${match.nextIntervalIndex} ` +
